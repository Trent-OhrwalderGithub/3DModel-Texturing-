...
public:
	...
	void lookAt(glm::vec3 eye, glm::vec3 center, glm::vec3 worldUp);

	void setPerspective(float vFOV, float aspectRatio, float nearDistance, float farDistance);

	void setTransform(const glm::mat4 transform) { m_worldTransform = transform; updateViewFromWorld(); }
	...
	glm::mat4 getProjectionView() { return m_projectionTransform * m_viewTransform; }
